...
- `BOOTFLASH`:
  - `primary` (default)
  - `secondary` (for LX2160A Clearfog-CX / Honeycomb backup flash)
...
- `*_xspi_*_*-*.img`:
  ...
  Bootloader only for installation to SPI Flash.

- `*_xspi_backup_*_*-*.img`:

  Bootloader only for installation to Clearfog-CX / Honeycomb backup SPI Flash.
...
### SPI Backup Boot (Clearfog-CX / Honeycomb only)

Clearfog-CX / Honeycomb boards have a seondary tiny flash memory for booting which can be activated by placing jumper on pin header J5018.
It requires a special tiny bootloader image generated by setting `BOOTFLASH=secondary` environment variable at build time.

For programming, boot from microSD, then load the tiny `*_xspi_backup_*_*-*.img` file and program it to secondary spi flash:

```
load mmc 0:1 $kernel_addr_r xspi_backup.img
# pass argument 0 if jumper J5018 was already place during power-on
sf probe 1
sf update $kernel_addr_r 0 0x400000
```

Change boot DIP switch on COM to off/off/off/off from numbers 1 to 4 (dip number 5 is not used. Notice the marking 'ON' on the DIP switch).

Finally place jumper on pin header J5018 to force carrier bord backup spi flash to chip-select zero causing SoC to boot from it.
